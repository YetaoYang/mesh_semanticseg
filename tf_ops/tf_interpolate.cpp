...
static const std::vector<std::vector<uint8_t>> map_label_to_color{
    {255, 255, 255}, {154, 205, 50}, {190, 190, 190}, {95, 158, 160}, {139, 34, 82},
    {255, 127, 0}, {238, 232, 170}, {85, 107, 47}, {34, 139, 34}, {205, 102, 0},
    {255, 255, 0}, {139, 26, 26}};
...
REGISTER_OP("InterpolateLabelWithColor")
    ...
    .Input("radius: float32")              // (num_dense_points, 3)
...
void interpolate_label_with_color_cpu(
    ...
    uint8_t *dense_colors, int knn, float radius) {
...
#pragma omp parallel for schedule(static)
    ...
    for (size_t j = 0; j < num_dense_points; ++j) {
        ...
        int max_count = 0;
        int most_frequent_label = 9;
        std::unordered_map<int, int> map_label_to_count;
        int label;
        for (size_t k = 0; k < knn_found; ++k) {
            if(candidate_dists[k] < radius){
...
class InterpolateLabelWithColorOp : public OpKernel {
   public:
    ...
    void Compute(OpKernelContext *context) override {
        ...
        OP_REQUIRES(context, radius_tensor.dims() == 0,
                    errors::InvalidArgument("knn must be an float scalar"));
        ...
        interpolate_label_with_color_cpu(
            ...
            dense_points, dense_labels, dense_colors, knn,radius);
    }
};
