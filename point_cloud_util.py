...
def load_features(fetures_path):
    features = np.loadtxt(fetures_path, delimiter=',')
...
def rotate_feature_point_list(list_data, feature_size=3, rotation_axis="z"):
    """ Randomly rotate the point clouds to augument the dataset
        rotation is per shape based along up direction
        Input:
          BxNx3 array, original batch of point clouds
    ...
    for k in range(len(list_data)):
        rotated_data = np.zeros(list_data[k].shape, dtype=np.float32)
        rotated_data[:, 3: 3 + feature_size] = list_data[k][:, 3: 3 + feature_size]
        rotation_angle = np.random.uniform() * 2 * np.pi
        cosval = np.cos(rotation_angle)
        sinval = np.sin(rotation_angle)
        if rotation_axis == "x":
